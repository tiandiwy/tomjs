...
const fileExists = require2('file-exists');
...
module.exports = async(type, model_name, paths) => {
    ...
    async function load_file(filename) {
        ...
        for (let i = 0; i < paths_len; i++) {
            ...
            if (await fileExists(all_filename) ||
                await fileExists(all_filename + '.js') ||
                await fileExists(all_filename + '.json') ||
                await fileExists(all_filename + '.node')) {
                return require(all_filename);
            }
        }
        return false;
    }

    async function endAddFile(filename, endAdd_arr, endAdd_arr_len) {
        let re = false;
        for (let i = 0; i < endAdd_arr_len; i++) {
            if (endAdd_arr[i].length > 0 && filename.toLowerCase().endsWith(endAdd_arr[i].toLowerCase())) {
                re = await load_file(filename.slice(0, 0 - endAdd_arr[i].length));
                if (re !== false) { break; }
            }
            re = await load_file(filename + endAdd_arr[i]);
            if (re !== false) { break; }
        }
        return re;
    }

    let re = await load_file(model_name);
    if (re === false) {
